...
## 2026-07-26 - Batching DOM Insertions with DocumentFragment
**Learning:** Appending multiple children consecutively to a DOM element inside high-frequency or repeated UI state updates causes layout thrashing and redrawn cycles. Offloading insertions to an in-memory `DocumentFragment` first and performing a single append to the DOM container yields a substantial speedup and prevents layout thrashing.
**Action:** Always batch DOM element insertions using `DocumentFragment` when dynamically building list interfaces.
